<div class="card card-body px-3 h-100 rounded-0 border-0">
			...
			<div class="d-flex flex-column gap-2 flex-grow-1">
				...
				<div class="d-flex flex-wrap gap-2 align-items-start">
					<div component="topic/labels" class="d-flex gap-1 flex-wrap {{{ if (!scheduled && (!pinned && (!locked && (!oldCid && !icons.length)))) }}}hidden{{{ end }}}">
						<span component="topic/scheduled" class="badge badge border-0border-gray-300 text-body {{{ if !scheduled }}}hidden{{{ end }}}">
							<i class="fa fa-clock-o"></i>
							[[topic:scheduled]]
						</span>
						<span component="topic/pinned" class="badge badge border-0border-gray-300 text-body {{{ if (scheduled || !pinned) }}}hidden{{{ end }}}">
							<i class="fa fa-thumb-tack"></i>
							{{{ if !pinExpiry }}}[[topic:pinned]]{{{ else }}}[[topic:pinned-with-expiry, {isoTimeToLocaleString(./pinExpiryISO, config.userLang)}]]{{{ end }}}
						</span>
						<span component="topic/locked" class="badge badge border-0border-gray-300 text-body {{{ if !locked }}}hidden{{{ end }}}">
							<i class="fa fa-lock"></i>
							[[topic:locked]]
						</span>
						<a href="{config.relative_path}/category/{oldCid}" class="badge badge border-0border-gray-300 text-body text-decoration-none {{{ if !oldCid }}}hidden{{{ end }}}">
					...
					<div class="d-flex flex-nowrap gap-1">
						<span class="badge border-0text-muted rounded-0 d-none d-lg-inline-block"><i class="fa fa-fw fa-user"></i> <span title="{postercount}">{humanReadableNumber(postercount)}</span></span>
						<span class="badge border-0text-muted rounded-0"><i class="fa-regular fa-fw fa-message"></i> <span title="{postcount}">{humanReadableNumber(postcount)}</span></span>
						<span class="badge border-0text-muted rounded-0"><i class="fa fa-fw fa-eye"></i> <span title="{viewcount}">{humanReadableNumber(viewcount)}</span></span>
					</div>
				</div>
			</div>

			<div class="d-flex gap-2 justify-content-end align-items-center hidden" component="topic/thumb/list">
				{{{ each thumbs }}}
				<a class="d-inline-block h-100" href="{./url}">
					<img class="rounded-0 h-100 bg-light" style="max-width: 4rem; object-fit: contain;" src="{./url}" />
				</a>
				{{{ end }}}
			</div>
		</div>
	...
	<div class="d-flex mb-2 gap-3">
		...
		<div class="d-none d-lg-block">
			<div class="sticky-top d-flex flex-column" style="top: 1rem; z-index: 1; max-width: 40px;">
