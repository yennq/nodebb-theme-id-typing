			...
			<div class="d-flex flex-column gap-2 flex-grow-1">
				...
				<div class="d-flex flex-wrap gap-2 align-items-start">
					<div component="topic/labels" class="d-flex gap-1 flex-wrap {{{ if (!scheduled && (!pinned && (!locked && (!oldCid && !icons.length)))) }}}hidden{{{ end }}}">
						<span component="topic/scheduled" class="badge badge border-0 text-body {{{ if !scheduled }}}hidden{{{ end }}}">
							<i class="fa fa-clock-o"></i>
							[[topic:scheduled]]
						</span>
						<span component="topic/pinned" class="badge badge border-0 text-body {{{ if (scheduled || !pinned) }}}hidden{{{ end }}}">
							<i class="fa fa-thumb-tack"></i>
							{{{ if !pinExpiry }}}[[topic:pinned]]{{{ else }}}[[topic:pinned-with-expiry, {isoTimeToLocaleString(./pinExpiryISO, config.userLang)}]]{{{ end }}}
						</span>
						<span component="topic/locked" class="badge badge border-0 text-body {{{ if !locked }}}hidden{{{ end }}}">
							<i class="fa fa-lock"></i>
							[[topic:locked]]
						</span>
						<a href="{config.relative_path}/category/{oldCid}" class="badge badge border-0 text-body text-decoration-none {{{ if !oldCid }}}hidden{{{ end }}}">
