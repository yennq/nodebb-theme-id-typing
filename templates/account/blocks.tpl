			...
			<ul component="blocks/search/list" class="dropdown-menu p-1 block-edit list-unstyled" role="menu">
				...
				<li component="blocks/search/match">
					<div class="dropdown-item rounded-1 d-flex flex-nowrap gap-2 justify-content-between align-items-center" role="menuitem">
						...
						<button class="btn-sm btn-sm-outline-danger text-nowrap {{{ if ./isBlocked }}}hidden{{{ end }}}" data-uid="{./uid}" data-action="block">[[user:block-user]]</button>
						<button class="btn-sm btn-sm-outline-primary text-nowrap {{{ if !./isBlocked }}}hidden{{{ end }}}" data-uid="{./uid}" data-action="unblock">[[user:unblock-user]]</button>
					</div>
				</li>
				{{{ end }}}
			</ul>
...
<div class="users row">
	...
	<div id="users-container" class="row row-cols-2 row-cols-md-3 row-cols-xl-4 g-3 users-container mb-4">
		...
		<div class="d-flex flex-column gap-1">
			...
			<button class="btn-sm btn-sm-outline-primary text-nowrap mx-auto" data-uid="{./uid}" data-action="unblock">[[user:unblock-user]]</button>
		</div>
		{{{ end }}}
